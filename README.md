# Parenttext Sri Lanka Pipeline

This project builds RapidPro flows for the Sri Lanka ParentText chatbot. It takes input from specific Google Sheets spreadsheets and produces RapidPro flow JSON files that are ready to upload to any RapidPro server.
...
To pull data from spreadsheet, convert to json and create flows:
```
python -m parenttext_pipeline.cli pull_data compile_flows
```
otherwise the 2 processes can be run separately:
